...
/*
*BulkTransferService.java: The requestKeyInfo takes the newnode hash value and 
*compares it with the keys of target node. After identitfying and adding, the keys to be
*transfered to the new node, it is removed from the source node
*
*/

public class BulkTransferService extends Service {
    ...
    public Map<String, String>  requestKeyInfo(Integer newNodeHash) {
        ...
        int pTargetkeyHash;
        System.out.println("[BulkTransferService] get keys less than " + newNodeHash); 
        for (String s : keysTarget) {
            pTargetkeyHash = HashFunction.getHashKey(s);
            if (pTargetkeyHash <= newNodeHash) {
                System.out.println("[BulkTransferService] adding key " + s);
