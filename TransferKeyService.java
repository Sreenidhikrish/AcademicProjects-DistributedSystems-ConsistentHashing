...
public class TransferKeyService extends Service {
    public TransferKeyService() {
        super();
    }
    ...
    private void addKeys(String kvArray){
        ...
        for(int i=0; i < kvJsonArray.length() ; i++){
            ...
            System.out.println("[TransferKeyService] adding key " + key + " value " + value);
        }
    }
    ...
    public void fetchData(NodeProperty targetNode) {
        ...
        try {
            ...
            outP.println("GET /context/bulktransfer/" + getNodeDetail().getNodeName() + " HTTP/1.0");
            outP.println("");
            outP.println("");
            outP.flush();
            //outP.close();
            ...
            System.out.println("[TransferKeyService] reading data from target node ..");
            ...
            while (!(readLine == null)) {
                ...
                System.out.println("[TransferKeyService]" + readLine);
            }
            
            System.out.println("[TransferKeyService] got response " + kvArray);
            if(kvArray != null){
                addKeys(kvArray);
            }
